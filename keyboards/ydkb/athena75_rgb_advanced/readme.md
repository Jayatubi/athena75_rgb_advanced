...
| **WFC**<br><img src="docs/apps/wfc.gif" width="256" alt="WFC"> | Simple tiled-model WFC（mxgmn 风格）：8×8×16 px，**Circuit / Pipes / Dungeon / Island** 四套经典主题；tile 由 `make_tiles.py` 直接绘制成 16×16 像素画。邻接规则只看一格、权重只看一张 tile，两者都是局部的，所以光靠它们只能得到处处合法、整体无意义的图案——**比一格大的结构必须在坍缩之前就定好**。每次重开先画一张“方案”：地牢是角点格阵上的房间与墙（横墙上开门、竖墙上留过道、最大的厅里放篝火），海岛是中心的一团陆地加恒为海的外圈（木屋靠岸、树林在内陆），电路和管道是若干条端子之间的直角走线。方案用求解器自己的语言写——角点掩码或出口掩码——所以它只是在加权抽签里替某一张 tile 加注，永远不会和约束冲突。←→（或 Enter → PLAN）调方案强度 OFF/HINT/SOME/FIRM/EXACT，EXACT 就是照着方案渲染。Space 切换 tileset，↑↓（或 `-`/`=`）调坍缩速度。未坍缩的格子按剩余候选 tile 的平均色填充。 |
