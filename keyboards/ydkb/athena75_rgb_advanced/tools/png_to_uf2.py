"""PNG -> QGF/UF2 打包工具 (真彩 RGB565, 全程不经 GIF 的 256 色量化)。
...
示例:
  python3 png_to_uf2.py boot boot.png                 # 60帧/16ms 淡入淡出 -> builds/boot.uf2
  python3 png_to_uf2.py boot boot.png --frames 60 --ms 16 --raw
  python3 png_to_uf2.py anim --keyframes              # 只输出关键帧, 固件实时补间
  python3 png_to_uf2.py anim --effect dissolve --compress
"""
import argparse
import glob
import os
...
TWEEN_MS = 100           # 过渡帧时长 (100ms 起面板才不串帧)
TWEEN = 1                # 每段补间帧数, 94*(1+TWEEN) 必须 <= 255

# --- anim 补间视觉参数 (与 make_montage.py 一致) ---
...
# boot: 单张 PNG -> 淡入淡出动画
# ============================================================================
def make_boot_frames(img, n, frame_ms):
    ...
    for i in range(n):
        ...
        frames.append((frame_to_rgb565(layer), frame_ms))  # 黑底合成 -> RGB565
    return frames
...
def cmd_boot(args):
    ...
    frames = make_boot_frames(img, args.frames, args.ms)

    qgf = build_qgf(frames, compress)
    addr = int(args.addr, 0)
    slot = BOOT_END - addr   # boot 槽容量 (默认 2MB), 防止溢出到 keyframe 区
    if len(qgf) > slot:
        raise SystemExit(f"QGF {len(qgf)}B 超出 boot 槽 ({slot}B)")
    uf2 = uf2_common.to_uf2(qgf, addr)
...
def load_emojis(src, align_text=False):
...
def cmd_anim(args):
    ...
    emojis = load_emojis(args.src, args.align_text)
    n = len(emojis)
    if n == 0:
        raise SystemExit(f"未找到 Icon_*.png (src={args.src})")
    ...
    if args.keyframes:
        ...
        tween_fn = TWEEN_FUNCS.get(args.effect)  # None => cut (无补间帧)
        ...
        for i in range(n):
            ...
            if tween_fn is not None:
                for j in range(1, args.frames + 1):
                    p = j / (args.frames + 1)
                    rgba_frames.append(tween_fn(a, b, p)); durations.append(args.tween)
    ...
    if args.base:
        ...
        base = "penta_kill_keyframes" + ("_aligned" if args.align_text else "") + ("_rle" if compress else "_raw")
    else:
        base = f"penta_kill_montage_12m_{args.effect}{args.frames}_t{args.tween}" \
               + ("_aligned" if args.align_text else "") + ("_rle" if compress else "")
    latest, stamped, _ = uf2_common.archive_bytes(uf2, base)

    total_s = sum(durations) / 1000
    ratio = f"  压缩率={len(qgf) / raw_bytes * 100:.0f}%" if compress else ""
    mode = "keyframes(MCU补间)" if args.keyframes else f"effect={args.effect} tween={args.tween}ms"
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="PNG -> QGF/UF2 (boot 淡入淡出动画 / anim 关键帧大动画)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    sub = ap.add_subparsers(dest="cmd", required=True)

    # --- boot: 单张 PNG -> 淡入淡出 -> boot 区 ---
    pb = sub.add_parser("boot", help="单张 PNG -> alpha 淡入淡出开机动画 (boot 区)")
    pb.add_argument("png", help="输入 PNG (任意尺寸, 会缩放到 128x128)")
    pb.add_argument("--frames", type=int, default=60, help="总帧数 (默认 60)")
    pb.add_argument("--ms", type=int, default=16, help="每帧时长 ms (默认 16)")
    pb.add_argument("--addr", default=hex(BOOT_ADDR),
                    help=f"目标 flash 地址 (默认 {hex(BOOT_ADDR)} = boot 区)")
    pb.add_argument("--base", default="boot", help="产物基名 (builds/<base>.uf2)")
    pb.add_argument("--raw", action="store_true", help="不压缩 (默认 RLE 压缩)")
    pb.set_defaults(func=cmd_boot)

    # --- anim: Icon_*.png 序列 -> 关键帧/补间 -> keyframe 区 ---
    pa = sub.add_parser("anim", help="Icon_*.png 序列 -> 关键帧/补间大动画 (keyframe 区)")
    pa.add_argument("--src", default=".", help="PNG 源目录 (含 Icon_*.png), 默认当前目录")
    pa.add_argument("--tween", type=int, default=TWEEN_MS, help="补间帧时长(ms)")
    pa.add_argument("--frames", type=int, default=TWEEN, help="每段补间帧数")
    pa.add_argument("--effect", default="dissolve",
                    choices=["ghost", "slide", "dissolve", "cut"],
                    help="补间效果: ghost(残影) / slide(纯滑动) / dissolve(溶解) / cut(硬切无补间)")
    pa.add_argument("--align-text", action="store_true",
                    help="对齐各帧底部 PENTA KILL 字样 (按白字质心平移, 需要 numpy)")
    pa.add_argument("--compress", action="store_true",
                    help="启用 QGF RLE 压缩 (固件软件解码, 显著缩小体积/加快烧录)")
    pa.add_argument("--keyframes", action="store_true",
                    help="只输出关键帧(每表情一帧), 补间交给固件 MCU 实时生成 "
                         "(默认不压缩, 固件直接 XIP 读取; 加 --compress 才用 RLE)")
    pa.add_argument("--base", default=None,
