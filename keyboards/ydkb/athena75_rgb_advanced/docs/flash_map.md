...
> 全部规划为 app 槽。关键帧动画改由 slot app + 其后的连续数据槽播放。
>
...
> 保留的 slot 数：首 slot 为程序，其后每 256 KiB 一个数据 slot，app 运行时通过
> `host_api.app_base()` 定位到 `app_base + 0x40000`。
...
> 数据 blob 随包一起安装，无需也不接受独立 `--data-uf2`：host_tool 从包内
